...
static float get_point_b_angle(enum angle_channel_e channel)
{
	...
	{
	case CHANNEL_ARML:
		return POINT_B_ARML_ANGLE;
	case CHANNEL_ARMR:
		return POINT_B_ARMR_ANGLE;
	case CHANNEL_BASE:
		return POINT_B_BASE_ANGLE;
	}
}
...
bool atuo_angle_calibra(void)
{
	...
	for (calibra_cnt = 0; calibra_cnt < sizeof(base_calibra_angle_map) / sizeof(uint16_t) - 1; calibra_cnt++)
	{ // <! base motor range 0~180
